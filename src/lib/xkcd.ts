...
export async function getByIndex(number: number): Promise<ComicResponse> {
	const response = await fetchWithTimeout(`${HOST}${number}/info.0.json`, {
		method: "GET",
	});
	if (!response.ok)
		throw new Error(
			`Could not fetch comic #${number}! HTTP error: ${response.statusText} (${response.status})`,
		);
...
export async function getLatest(): Promise<ComicResponse> {
	const response = await fetchWithTimeout(`${HOST}/info.0.json`, {
		method: "GET",
	});
	if (!response.ok)
		throw new Error(
			`Could not fetch latest comic! HTTP error: ${response.statusText} (${response.status})`,
		);
	const data: ComicResponse = await response.json();
	return data;
}
